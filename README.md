...
После перезапуска контейнеров необходимо применить миграции. Для этого необходимо подключиться к терминалу контейнера с бэкендом, выполнив команду:
```sh
$ docker exec -it star-burger-backend bash
```

После подключения к контейнеру строка-приглашение примет примерно такой вид: `root@639b5286df56:/backend#`

Для применения миграций ввести команду:
```sh
$ python manage.py migrate
```

Выйти из терминала контейнера нажав комбинацию клавиш `Ctrl+D`.
